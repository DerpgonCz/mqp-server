...
Room.prototype.updateLobbyServer = function(song, dj, callback) {
	...
	request.post({url: 'https://api.musiqpad.com/pad/' +  this.roomInfo.slug, json: postData, headers: {
			'Content-Type': 'application/json',
			'apikey': config.apis.musiqpad.key
		}
	}, function(err, resp, body){
		if(err){
			console.log('Lobby Update errored.', err);
		} else if([200, 201, 202, 203, 204, 205, 206, 207, 208, 226].indexOf(resp.statusCode) === -1){
			console.log('Lobby Update returned unknown status code.', resp);
		}
		if(callback) callback();
	});
	// try {
