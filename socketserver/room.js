...
var Room = function (socketServer, options) {
    ...
    DB.getRoom(this.roomInfo.slug, function (err, data) {
        ...
        if (err && !err.notFound) {
            story.error('Room', 'Get room errored', {attach: err});
            return;
        }
...
Room.prototype.makeOwner = function () {
    ...
    DB.getUser(this.roomInfo.ownerEmail, function (err, data) {
        if (err == 'UserNotFound') {
            story.warn('Room', 'Cannot make room owner, user doesn\'t exist yet');
            that.data.roles.owner = [];
            return;
        }
        if (err) {
            story.error('Room', 'Cannot make room owner', {attach: err});
            return;
        }

        if (typeof data.uid !== 'number') {
            story.warn('Room', 'Cannot make room owner, USERID isn\'t a number');
            return;
        }

        story.info('Room', 'Granting ' + data.un + ' (' + data.uid + ') Owner permissions');
...
Room.prototype.updateLobbyServer = function (song, dj, callback) {
    ...
    request.post({
        url: 'https://api.musiqpad.com/pad/' + this.roomInfo.slug, json: postData, headers: {
            'Content-Type': 'application/json',
            'apikey': config.apis.musiqpad.key
        }
    }, function (err, resp, body) {
        if (err) {
            story.error('lobbyupdate', 'Update errored', {attach: err});
        } else if ([200, 201, 202, 203, 204, 205, 206, 207, 208, 226].indexOf(resp.statusCode) === -1) {
            story.warn('lobbyupdate', 'Update returned unknown status code', {attach: [resp]});
        }
        if (callback) callback();
    });
