...
//					MP.emojiReplace($('<span class="umsg"></span>').text(msg).prop('outerHTML')) + '</div></div></div>'
        ...
        chatCommands: {
            ...
            swap: {
                ...
                exec: function (arr) {
                    ...
                    var users = MP.api.room.getUsers();
                    var user1 = users.filter(function (a) {
                        return a.un == arr[0].substring(1);
                    })[0];
                    var user2 = users.filter(function (a) {
                        return a.un == arr[1].substring(1);
                    })[0];
            ...
            forcereload: {
                description: 'Forces the room to reload pages',
                staff: true,
                exec: function (arr) {
                    console.log(1);
                    API.sendSocket({type: 'forceReload'});
                }
            },
            ...
            padrestart: {
                ...
                staff: true,
                exec: function (arr) {
                    API.sendSocket({type: 'padRestart'});
                }
            },

            settweeter: {
                description: 'Toggles autotweeter',
                staff: true,
                exec: function (arr) {
                    API.sendSocket({type: 'twitterToggle', data: {val: arr[1] === 'on'}});
                }
            },
            ...
            sendtweet: {
                ...
                exec: function (arr) {
                    if (arr.length < 2) {
                        return API.chat.log('No message, no tweet.');
                    } else {
                        arr.shift();
        ...
        djQueueModSwap: function (uid1, uid2, callback) {
            ...
            obj.id = MP.addCallback(obj.type, function (err, data) {
                if (err) {
                    if (callback) callback(err);
                    console.log('Could not swap users in dj queue: ' + err);
                    return;
                }
...
//		if (MP.user && data.users[i].uid == MP.user.uid) MP.user.role = data.users[i].role;
    ...
    var addPlaylistButton = function (e) {
        ...
        MP.playlistCreate($input.val(), function (err, data) {
            delete window.PLAYLISTCREATEINPROGRESS;
            if (err) {
                MP.makeAlertModal({content: 'There was a problem adding the playlist: ' + err});
                return;
            }
    ...
    $(window).on('load', function () {
        MP.session.oldPageTitle = document.title || MP.session.roomInfo.title;
        var win = $(this);
        var settings = JSON.parse(localStorage.getItem("settings"));
